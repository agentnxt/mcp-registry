...
export function registerAccountsTools(server, getClient) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    }, async ({ target, verificationRecordId }) => getClient().delete(`/my-account/identities/${target}`, { data: { verificationRecordId } }).then((r) => r.data));
    ...
    }, async (a) => getClient().get(`/my-account/social-identity-access-token/${a.target}`).then((r) => r.data));
    ...
    }, async (a) => getClient().get(`/my-account/enterprise-sso-identity-access-token/${a.ssoConnectorId}`).then((r) => r.data));
    // ═══════════════════════════════════════════════════════════════
    ...
    }, async ({ verificationId, name }) => getClient().patch(`/my-account/mfa-verifications/${verificationId}`, { name }).then((r) => r.data));
    ...
    }, async ({ verificationId, verificationRecordId }) => getClient().delete(`/my-account/mfa-verifications/${verificationId}`, { data: { verificationRecordId } }).then((r) => r.data));
    tool("account_generate_totp_secret", "Generate a new TOTP secret for binding [Accounts API]", {
        type: "object", properties: {},
    }, async () => getClient().post("/my-account/mfa-verifications/totp-secret").then((r) => r.data));
    tool("account_generate_backup_codes", "Generate new backup codes [Accounts API]", {
        type: "object", required: ["verificationRecordId"],
        properties: { verificationRecordId: { type: "string" } },
    }, async (a) => getClient().post("/my-account/mfa-verifications/backup-codes/generate", a).then((r) => r.data));
    tool("account_get_backup_codes", "Get the current user's backup codes [Accounts API]", {
        type: "object", properties: {},
    }, async () => getClient().get("/my-account/mfa-verifications/backup-codes").then((r) => r.data));
}
